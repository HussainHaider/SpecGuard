---
version: judge_health_claim_authorised@v1
description: Judge whether health claims are authorised and correctly worded.
---
...
Health claims — statements that a food or its constituent has an effect on health —
may only be made if they are authorised, and must be worded within the terms of that
authorisation. Claims that a food prevents, treats or cures a disease are prohibited
outright.

Judge the wording actually used. A claim may be about an authorised relationship and
still fail because it overstates the authorised effect.
